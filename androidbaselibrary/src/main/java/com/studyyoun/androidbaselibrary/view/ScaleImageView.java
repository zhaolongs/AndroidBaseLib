...
public class ScaleImageView extends AppCompatImageView {
    ...
    //控件是否能平移
    private boolean mCanTranslate = true;
    //控件是否能旋转
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        mGestureDetector1 = new GestureDetector(context, new GestureDetector.SimpleOnGestureListener() {
            ...
            public boolean onDoubleTap(MotionEvent e) {
                ...
                LogUtils.d("aFloat " + aFloat);
                if (mCurrentScale == 1.0) {
                    mCurrentScale = 2.0f;
                    mCurrentMatrix.postScale(2.0f, 2.0f, mBoundRectF.width() / 2, mBoundRectF.height() / 2);
                    setImageMatrix(mCurrentMatrix);
                } else {
                    mCurrentScale = 1.0f;
                    mCurrentMatrix.postScale(1.0f, 1.0f, mBoundRectF.width() / 2, mBoundRectF.height() / 2);
                    setImageMatrix(mCurrentMatrix);
                }
    ...
    private long mPrreDoubleClickTime;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (((int)mBoundRectF.width()) > getWidth()  || ((int)mBoundRectF.height()) > getHeight()) {
        ...
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_DOWN:
                mPrreDoubleClickTime = System.currentTimeMillis();
                mCanTranslate = true;
                mCanRotate = false;
                mCanScale = false;
                //记录单点触控的上一个单点的坐标
                ...
                if (1 == count) {
                    ...
                    if (secClick - firClick < interval) {
                        ...
                        final int intrinsicHeight = (int) mBoundRectF.height();
                        final int intrinsicWidth = (int) mBoundRectF.width();
                        ...
                        final RectF matrixRectF = new RectF();
                        matrixRectF.set(0, 0, drawable.getIntrinsicWidth(), drawable.getIntrinsicHeight());
                        mCurrentMatrix.mapRect(matrixRectF);


                        float deltaX=1.0f;
                        float deltaY=1.0f;
                        if (((int)matrixRectF.width()) <= getWidth()||((int)matrixRectF.height()) <=getHeight()) {
                            if (matrixRectF.width() < getWidth()) {
                                deltaX = getWidth() / 2 - matrixRectF.right + matrixRectF.width() / 2;
                            }
                            if (((int)matrixRectF.height()) < getHeight()) {
                ...
                if (pointerCount==1) {
                    ...
                    int height = getHeight();
                    int width = getWidth();
                    int boundHeight = (int) mBoundRectF.height();
                    int boundWidth = (int) mBoundRectF.width();
                    if ( boundHeight<= height&& boundWidth <= width) {
                        mCanTranslate=false;
                    }
                }
                secClick = 0;
    ...
    protected void backScale() {
        ...
        mCurrentMatrix.postScale(1.0f, 1.0f, mBoundRectF.centerX(), mBoundRectF.centerY());
    }
