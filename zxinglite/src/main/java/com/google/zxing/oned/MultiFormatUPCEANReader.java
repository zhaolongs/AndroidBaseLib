...
public final class MultiFormatUPCEANReader extends OneDReader {
  ...
  public MultiFormatUPCEANReader(Map<DecodeHintType,?> hints) {
    ...
    if (possibleFormats != null) {
      ...
      } else if (possibleFormats.contains(ZBarcodeFormat.UPC_A)) {
        readers.add(new UPCAReader());
      }
      if (possibleFormats.contains(ZBarcodeFormat.EAN_8)) {
        readers.add(new EAN8Reader());
      }
      if (possibleFormats.contains(ZBarcodeFormat.UPC_E)) {
  ...
  public Result decodeRow(int rowNumber,
    ...
    for (UPCEANReader reader : readers) {
      ...
      boolean ean13MayBeUPCA =
          result.getBarcodeFormat() == ZBarcodeFormat.EAN_13 &&
              result.getText().charAt(0) == '0';
      @SuppressWarnings("unchecked")      
      Collection<ZBarcodeFormat> possibleFormats =
          hints == null ? null : (Collection<ZBarcodeFormat>) hints.get(DecodeHintType.POSSIBLE_FORMATS);
      boolean canReturnUPCA = possibleFormats == null || possibleFormats.contains(ZBarcodeFormat.UPC_A);
